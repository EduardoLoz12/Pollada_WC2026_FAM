...
Every push must increment `<div class="footer-version">vX.Y.Z</div>` in index.html (v1.3.2 as of 2026-07-04). It's how Eduardo verifies which build is live. As of v1.2.x, each segment stays a single digit (1.2.9 → 1.3.0, not 1.2.10) — Eduardo's explicit preference, don't let any segment grow to two digits.
