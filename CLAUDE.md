...
Every push must increment `<div class="footer-version">vX.Y.Z</div>` in index.html (v1.2.8 as of 2026-06-30). It's how Eduardo verifies which build is live. As of v1.2.x, each segment stays a single digit (1.2.9 → 1.3.0, not 1.2.10) — Eduardo's explicit preference, don't let any segment grow to two digits.
...
- "Mis Pronósticos" tab: type name → saved picks, knockout-stage rows first (always visible), Fase de Grupos rows collapsed by default (`▸` reuses the same `toggleGroup()` as the join-modal's day groups). Each row shows two separate ✅/❌ badges when finished — one for the winner pick, one for the exact-score guess (only shown if a score was entered) — don't collapse them back into one combined icon, they test different things. Remaining-predictions alert and the form in the join/quick modals all use `getEditablePhases()`, not just the single "active" phase.
