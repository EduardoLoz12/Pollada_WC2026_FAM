...
Every push must increment `<div class="footer-version">vX.Y.Z</div>` in index.html (v1.3.9 as of 2026-07-19). It's how Eduardo verifies which build is live. As of v1.2.x, each segment stays a single digit (1.2.9 → 1.3.0, not 1.2.10) — Eduardo's explicit preference, don't let any segment grow to two digits.
...
- `renderLeaderboard()` / `calcPoints()` — points (`POINTS` in config.js, stage-aware as of v1.2.6): `correct_result_group` (2 pts, **locked** — applies only to already-played GROUP_STAGE picks, never bump this or it retroactively reinflates history) + `correct_result` (3 pts, knockout stage going forward) + `exact_score_bonus` (2 pts, any stage, on top of a correct result) + 10/5/5 (champion/runner-up/scorer special bets). There's no separate "knockout bonus" anymore — it was folded into `correct_result`. Points only count when match status is FINISHED. **Gotcha:** changing any `POINTS` value recalculates ALL historical predictions live (no point-in-time snapshot) — a global bump retroactively re-scores already-played matches too; use the `_group` vs general split pattern above if a points change should only apply going forward.
  - **v1.3.9 bug (2026-07-19):** `calcPoints()` never actually awarded the 10/5/5 special-bet points despite this being documented all along — `renderLeaderboard()` only ever called `calcPoints(p.id)` with no special-bets addition anywhere in the codebase. Went undetected all tournament because it only matters once the FINAL is decided. Found + fixed the same day Spain won (real champion, Mbappé real top scorer with 10 goals) — directly affects who wins the S/500 prize, so this was verified against live prod data (Node script reproducing the exact JS logic) before pushing, not just eyeballed.
  - **Special-bet text was never standardized** — typed free-form all tournament (`"Francia"` vs `"France"`, `"Mbappe"` vs `"Kylian Mbappé"`, plus outright typos like `"Fancia"`, `"Mesi"`). `teamsMatch()`/`scorerMatches()` (app.js) normalize accents/case, resolve a hardcoded `TEAM_ALIASES` ES→EN dict (only covers names that differ from the English string, not accent-only ones — NFD-strip already handles those), and for scorers do a token-level substring match so a bare last name matches the full API name. Typos that aren't in the alias dict (e.g. `"Fancia"`) still fail silently — acceptable since a wrong pick failing to match is correct behavior, but don't assume 100% recall for garbled entries nobody's checked.
  - `python/export_excel.py`'s `calc_points()` had the identical gap (never scored special bets) — ported the same `normalize_name`/`teams_match`/`scorer_matches`/alias-dict logic there too so the Excel export doesn't drift from the live leaderboard (see "Known loose ends" below, this specific drift risk is now resolved but keep both in sync if the matching logic changes again).
...
- 2026-07-08: R16 predictions actually WERE saving fine (verified via direct query, 54 rows across 8 participants) when a "no se guardan los de Octavos" report came in — real root cause was different: `updateScore()` (exact-score inputs) didn't set `pred_result`, only the separate H/A button click did, so anyone who typed a score without also clicking the winner button silently lost that pick (filtered out of `predRows` in `submitPredictions()` for having no `.result`). Fixed by having `updateScore()` auto-derive/select the winner from the score itself — a knockout match's exact score always implies a winner, so this is safe and not just a workaround. Reproduced live with a real participant (Nelly, "guardamos 2 de 8") before shipping the fix.
- 2026-07-08: Switzerland 0-0 Colombia (penalties) got stuck on `winner: DRAW` even though football-data's API already had it correctly resolved (`HOME_TEAM`, regularTime 0-0, penalties 4-3) — the winner-lock added the same day (previous bullet) blocked the correction once a bad first write landed. Same "first-write-wrong self-locks" trap as the Colombia-Ghana score case, just hit `winner` this time; needs the same manual-patch treatment, the lock can't self-heal it.
- 2026-07-19: **special bets never counted toward points at all** — `calcPoints()` (app.js) only ever scored match predictions; the "10/5/5 for champion/runner-up/scorer" in this doc was aspirational, not real, until fixed the same day Spain won the final (real champion) and the app was asked to tally final results. See the v1.3.9 bullet under "Frontend structure" above for the full writeup (fuzzy ES/EN team matching, scorer last-name matching, `export_excel.py` synced too). Directly affects the S/500 prize winner, so this was the highest-stakes fix of the project — verified against live prod data before pushing.
...
- `python/export_excel.py` duplicates the `POINTS`/`calc_points` logic from `app.js` by hand (no shared module) — if the scoring formula changes again, update both or the Excel export will silently drift from the live leaderboard. (Special-bet scoring + fuzzy name matching synced 2026-07-19, but the duplication itself remains — still two places to update.)
